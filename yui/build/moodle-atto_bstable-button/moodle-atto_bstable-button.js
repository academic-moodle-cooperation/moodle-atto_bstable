YUI.add('moodle-atto_bstable-button', function (Y, NAME) {
 ...
 * @copyright  based on code by 2013 Damyon Wiese  <damyon@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

/**
 * @module moodle-atto_bstable-button
 */

/**
 * Atto text editor bsbstable plugin.
 *
 * @namespace M.atto_bstable
 * @class Button
 * @extends M.editor_atto.EditorPlugin
 */

var COMPONENT = 'atto_bstable',
    EDITTEMPLATE = '' +
        '<form class="{{CSS.FORM}}">' +
            '<label for="{{elementid}}_atto_bstable_caption">{{get_string "caption" component}}</label>' +
            '<input class="{{CSS.CAPTION}} fullwidth" id="{{elementid}}_atto_bstable_caption" required />' +
            '<br/>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_bstable_headers" class="sameline">{{get_string "headers" component}}</label>' +
            '<select class="{{CSS.HEADERS}}" id="{{elementid}}_atto_bstable_headers">' +
    ...
    TEMPLATE = '' +
        '<form class="{{CSS.FORM}}">' +
            ...
            '</select>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_bstable_rows" class="sameline">{{get_string "numberofrows" component}}</label>' +
            '<input class="{{CSS.ROWS}}" type="number" value="3" id="{{elementid}}_atto_bstable_rows" size="8" min="1" max="50"/>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_bstable_columns" class="sameline">{{get_string "numberofcolumns" component}}</label>' +
            '<input class="{{CSS.COLUMNS}}" type="number" value="3" id="{{elementid}}_atto_bstable_columns" size="8" min="1" max="20"/>' +
            '<br/>' +
            '<div class="mdl-align">' +
                '<br/>' +
...
Y.namespace('M.atto_bstable').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
